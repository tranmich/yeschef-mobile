...
const UserCommunityPostsScreen = ({ navigation }) => {
  ...
  const loadUserPosts = async () => {
    try {
      ...
      // Get all community recipes to see what's available
      const allRecipesResponse = await YesChefAPI.get('/api/community/recipes?limit=50');
      console.log('🔍 All community recipes response:', allRecipesResponse);
      
      if (allRecipesResponse.success) {
        console.log('🔍 All community recipes:', allRecipesResponse.recipes);
        
        // Show what each recipe's shared_by field looks like
        allRecipesResponse.recipes.forEach((recipe, index) => {
          console.log(`🔍 Recipe ${index + 1}: shared_by="${recipe.shared_by}", user="${recipe.user}", title="${recipe.title || recipe.community_title}"`);
        });
        
        // Filter recipes shared by current user
        // Try multiple possible matching criteria
        const userEmail = currentUser?.email;
        const userName = currentUser?.name;
        const userRecipes = allRecipesResponse.recipes.filter(recipe => {
          const matches = recipe.shared_by === userEmail ||
                         recipe.user === userEmail ||
                         recipe.shared_by === userName ||
                         recipe.user === userName ||
                         recipe.shared_by === `${userName}Chef` ||
                         recipe.user === `${userName}Chef` ||
                         recipe.shared_by?.includes(userEmail?.split('@')[0]) ||
                         recipe.user?.includes(userEmail?.split('@')[0]);
          
          if (matches) {
            console.log('🎯 Found matching recipe:', recipe.title || recipe.community_title);
          }
          
          return matches;
        });
        
        console.log('🔍 Found user recipes:', userRecipes.length);
